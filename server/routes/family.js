...
router.get("/", requireAuth, async (req, res) => {
  try {
    ...
    const [members, items, events, rewards, doneLog, redeemReqs, settings] = await Promise.all([
      ...
      supabase.from("families").select("rate, period_start, period_days, spent_points, categories, event_categories, family_name").eq("id", fid).single(),
    ]);
...
router.put("/settings", auth, async (req, res) => {
  const allowed = ["rate", "period_start", "period_days", "spent_points", "categories", "event_categories"];
  const update = {};
