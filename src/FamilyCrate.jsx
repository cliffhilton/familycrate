                      ...
                      return (
                        <div key={ds} className="pcol" style={{height:totalGridHeight,position:"relative",flex:"1",minWidth:showWeekends?80:110}} onClick={e=>{if(e.target===e.currentTarget){setSelDate(ds);setDayView("day");}}}>
                          {laid.map(block=>{const n2=Math.min(block.totalCols,3),colW2=1/n2,leftPct=block.col*colW2*100,widthPct=colW2*100;return(
                            <div key={block.id} className={`tblock${block.title?.startsWith("🎉")?" tblock-reward":""}`} style={{top:block.top,height:block.height,left:`calc(${leftPct}% + 1px)`,width:`calc(${widthPct}% - 3px)`,background:block.title?.startsWith("🎉")?"var(--gold)":`${block.color}22`,borderLeftColor:block.color,zIndex:block.col+1}} onClick={e=>{e.stopPropagation();if(block.kind==="event") setViewModal({event:events.find(ev=>ev.id===block.evId)});else setViewModal({item:items.find(i=>i.id===block.itemId),memberId:block.memberId,ds});}}>
                              {(()=>{const isReward=block.title?.startsWith("🎉");return(<div className="tblock-title" style={{color:isReward?"#fff":block.color,fontSize:10,fontWeight:isReward?700:500,paddingRight:block.isGoogle?18:block.kind==="item"?18:0}}>{block.title}</div>);})()}
                              ...
                              {block.height>26&&<div className="tblock-time" style={{color:block.title?.startsWith("🎉")?"rgba(255,255,255,0.85)":block.color,fontSize:8}}>{block.time}</div>}
