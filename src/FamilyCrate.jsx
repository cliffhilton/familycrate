...
function FamilyCrate({ apiData, onLogout }) {
  ...
  function buildBlocks(mid,ds){
    ...
    const blocks=[];
    colEvs.forEach(ev=>{if(ev.source==="google"&&!["admin","mom","dad"].includes(m.role))return;
              ...
              <div className="cal-inner">
                ...
                {upcomingEvs.map((ev,i)=>(
                  <div key={i} className="uev" onClick={()=>{setEModal({event:ev});setCalOpen(false);}}>
                    ...
                    {ev.source==="google"&&<span style={{fontSize:8,background:"#4A90D9",color:"#fff",borderRadius:3,padding:"1px 4px",fontWeight:700}}>G</span>}
                      ...
                      return (
                        <div key={ds} className="pcol" style={{height:totalGridHeight,position:"relative",flex:"1",minWidth:showWeekends?80:110}} onClick={e=>{if(e.target===e.currentTarget){setSelDate(ds);setDayView("day");}}}>
                          {laid.map(block=>{const n2=Math.min(block.totalCols,3),colW2=1/n2,leftPct=block.col*colW2*100,widthPct=colW2*100;return(
                            <div key={block.id} className={`tblock${block.isReward?" tblock-reward":""}`} style={{top:block.top,height:block.height,left:`calc(${leftPct}% + 1px)`,width:`calc(${widthPct}% - 3px)`,background:block.isReward?"#C49A3C":`${block.color}22`,borderLeftColor:block.color,zIndex:block.col+1}} onClick={e=>{e.stopPropagation();if(block.kind==="event") setViewModal({event:events.find(ev=>ev.id===block.evId)});else setViewModal({item:items.find(i=>i.id===block.itemId),memberId:block.memberId,ds});}}>
                              {(()=>{const isReward=block.isReward;return(<div className="tblock-title" style={{color:isReward?"#fff":block.color,fontSize:10,fontWeight:isReward?700:500,paddingRight:block.isGoogle?18:block.kind==="item"?18:0}}>{block.title}</div>);})()}
                              ...
                              {block.height>26&&<div className="tblock-time" style={{color:block.isReward?"rgba(255,255,255,0.85)":block.color,fontSize:8}}>{block.time}</div>}
