...
export default function FamilyCrate() {
  ...
  // CRUD
  const saveMember=(id,p)=>id?setMembers(m=>m.map(v=>v.id===id?{...v,...p}:v)):setMembers(m=>[...m,{id:uid(),color:"#3A6ACC",photo:null,email:"",phone:"",...p}]);
  const delMember=id=>{setMembers(m=>m.filter(v=>v.id!==id));setItems(i=>i.map(v=>({...v,assignedTo:v.assignedTo.filter(x=>x!==id)})));setFilterMids(p=>{const n=new Set(p);n.delete(id);return n;});};
